...
app.get('/api/walkers/summary', async (req, res) => {
  try {
    const [summary] = await db.execute(`SELECT u.username AS walker_username,
                                      ...
                                      (SELECT COUNT(*) FROM WalkApplications WHERE WalkApplications.walker_id = u.user_id AND (SELECT WalkRequest.status FROM WalkRequest WHERE ) = 'completed') AS completed_walks
                                      FROM Users AS u
                                      WHERE u.role = 'walker';
                                      `);
    res.json(summary);
